...
public class SecurityController {
	...
	public void initBinder(WebDataBinder binder) {
		...
		dateFormat.setLenient(false); // yyMMdd형식이 아닌 경우 에러를 발생
		
		binder.registerCustomEditor(Date.class,"birthday",
				new CustomDateEditor(dateFormat, true));
		// birthday=991225와 같은 형식으로 데이터가 들어오는 경우 수행되는 커스텀에 디터등록
	}
	...
	public String register(
			...
			// BindingResult
			//	- 유효성검사 결과를 저장하는 객체
			//	- forward시 자동으로 jsp에게 전달되며, form태그 내부에 에러내용을 바인딩할 때 사용됨.
		...
		// 유효성검사 통과시 비밀번호 정보는 암호화하여, 회원가입 진행
		String encrptedPassword = passwordEncoder.encode(member.getUserPwd());
		member.setUserPwd(encrptedPassword);
	 ...
	 *  - Authorities : 인증된 사용자가 권한을 저장하는 객체 
	 */
