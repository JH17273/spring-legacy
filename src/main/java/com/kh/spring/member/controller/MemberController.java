...
public class MemberController {
	...
	/*
	 * Spring의 DI(Dependency Injection) - 의존성 주입 - 어플리케이션을 구성하는 객체를 개발자가 직접 생성하는 게
...
//	}
	...
	/*
	 * 커맨드 객체 방식 메소드의 매개변수로 vo클래스 타입을 지정하는 경우, 요청 전달값의 name속성과 일치하는 VO클래스 필드의
	 * Setter함수를 호출하여 바인딩
	 * 
	 * 매개변수의 클래스와 일치하는 클래스의 기본생성자 호출
	 * 파라미터의 key값과 클래스의 필드명이 일치하는 경우 setter 호출
	 */

//	@RequestMapping(value = "/member/login", method = RequestMethod.POST)
//	public String login(Member m) {
//		// Member m2 = Member.builder().userId("1234").userPwd("12").buld()
//		// -> build 패턴의 방식(구식)
//		
...
//		throw new RuntimeException("예외발생");
		...
		if(result > 0){
			...
			ra.addFlashAttribute("alertMsg","회원정보 수정 실패");
